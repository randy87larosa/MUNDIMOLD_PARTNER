...
class AccountBankStatementLine(models.Model):
    ...
    journal_payment_manual_id = fields.Many2one(comodel_name='account.journal', domain="[('type', '=', 'bank')]", string='Diario pago man.')
    journal_payment_id = fields.Many2one(comodel_name='account.journal', compute='_compute_payment_journal', store=True, string='Diario pago')
    ...
    @api.depends('journal_payment_manual_id', 'journal_id')
    def _compute_payment_journal(self):
        for item in self:
            item.journal_payment_id = item.statement_id.journal_id
            if item.journal_payment_manual_id:
                item.journal_payment_id = item.journal_payment_manual_id

...
class ProjectTaskNativeScheduler(models.Model):
    ...
    def _scheduler_plan_start_calc(self, project):
        ...
        #_logger.error("ENTRANDO EN SCHEDULER")
        #_logger.error("ENTRANDO EN SCHEDULER")
        #_logger.error("ENTRANDO EN SCHEDULER")
        
        domain = [('project_id', '=', project.id), ('schedule_mode', '=', 'manual')]
        for task in self.env['project.task'].sudo().search(domain):
            ...
            if len(task.child_ids) <= 0:
                ...
                #CAMBIAMOS WORK
                for resource in task.task_resource_ids:
                    num_int = resource.resource_id.calendar_id.get_work_hours_count(task.date_start, task.date_end, True)
                    break
                if num_int > 0:
                    task.plan_duration = num_int * 60 * 60
            
                #CAMBIAMOS MODO
                task.schedule_mode = 'auto'
                task.constrain_type = 'mso'
                task.constrain_date = task.date_start
        ...
        for task in tasks_list:
            ...
            #    })
            #else:
            #    task_resource = task_resource_ids[0]
            #
            #    task_res.append({
            #        "calendar_id": task_resource.resource_id.calendar_id,

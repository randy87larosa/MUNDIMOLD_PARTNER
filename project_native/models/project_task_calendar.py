...
class ProjectTaskNativeCalendar(models.Model):
    ...
    def _get_work_intervals(self, _resource_id, work_already_day, work_time_day, _load_factor):
        ...
        _logger.error("ENTRANDO EN WORK ITERVALS")
        #_logger.error(_resource_id)

        #resource = self.env['resource.resource'].sudo().search([('id', '=', _resource_id)])
        
        #_logger.error(resource.time_efficiency)
            
        work_time = work_time_day[_resource_id] * _load_factor
        
        #_logger.error("WORK TIME")
        _logger.error(work_time)
        _logger.error(work_time_already)
